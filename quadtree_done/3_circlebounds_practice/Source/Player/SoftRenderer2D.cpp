...
void SoftRenderer::DrawGrid2D()
{
	...
	ScreenPoint gridSouthWest = ScreenPoint::ToScreenCoordinate(_ScreenSize, minGridPos - viewPos);

	for (int ix = 0; ix < xGridCount; ++ix)
	{
		_RSI->DrawFullVerticalLine(gridSouthWest.X + ix * _Grid2DUnit, gridColor);
	}

	for (int iy = 0; iy < yGridCount; ++iy)
	{
		_RSI->DrawFullHorizontalLine(gridSouthWest.Y - iy * _Grid2DUnit, gridColor);
	}
...
void SoftRenderer::Render2D()
{
	...
	size_t culledObjectCountQuad = 0;
	...
	CK::Rectangle cameraRectangleBound(_GameEngine.GetCamera().GetRectangleBound());
	CK::Rectangle cameraRectangleBoundInWorld(cameraRectangleBound);
	Vector2 cameraPos = _GameEngine.GetCamera().GetTransform().GetPosition();
	cameraRectangleBoundInWorld.Min += cameraPos;
	cameraRectangleBoundInWorld.Max += cameraPos;

	std::vector<std::string> goKeys;
	_GameEngine.GetQuadTreeRoot().Query(cameraRectangleBoundInWorld,goKeys);
	goKeys.push_back(GameEngine::PlayerKey);
	culledObjectCountQuad = totalObjectCount - goKeys.size();
	...
	_RSI->PushStatisticText("Culled by QuadTree : " + std::to_string(culledObjectCountQuad));
